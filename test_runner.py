...
FORMAT = "markdown"
TIMEOUT = 60
COMMON_ARG_PYTHON = "python3 fibonacci.py --algo {algo} {n}"
...
def run_single(n: int, typ: int) -> float:
    ...
    if math.isnan(LAST_RUN_TRACKER[typ]):
...
def build_row(n: int) -> str:
    ...
    if FORMAT == "markdown":
        return f"| {n:<4} | {iterative.center(8, ' ')} |" \
               f" {recursive.center(8, ' ')} | {dynamic_programming.center(8, ' ')} |"
